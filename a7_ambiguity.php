...
<?php
include "connect.php";
$query1=array("select","where",",","class","there","find","enter","by","from","who","to","received","receiving","receives","receive","obtains","obtain","obtaining","obtained","what","in","show","with","then","than","tell","number","no","and","also","me","list","out","which","is","the","a","an","for","are","of","has","having","have","had","been");
...
<?php }
	...
	{ 
		...
		{
			$arrayy= $relation_attribute[$s].".".$attributes[$s];
			$array_m[$fd]=$arrayy;
			$fd++;
		}
		}
...
$flgh=0;
$t=0;
	for($ip=0;$ip<count($ambiguous);$ip++)
{
	for($wq=0;$wq<count($attribute_name);$wq++)
	{	
	if($ambiguous[$ip]===$attribute_name[$wq])
	{
?>
		<td>
		<?php $attribute_added_ambi[$t]=$ambiguous[$ip];
		$t++;	
		?> 
		</td>
	<?php 
	...
	if(count($attribute_added_ambi)==0){
		echo "No ambiguity";
	}
		else
		{
	...
	<?php
	for($io=0; $io<count($attribute_added_ambi);$io++)
	{
	?>
	<tr>
	<td><?php echo $attribute_added_ambi[$io]; ?></td>
	</tr><?php
	}
	?>
	</table> 
...
<center></br></br><input type="submit" name="next8" value="Next"/>
</form>
</body>
</html>